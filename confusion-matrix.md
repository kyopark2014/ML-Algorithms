...
- 전체 메일 100개중에 실제 스펨이 60개이고, 스펨으로 판정한 메일 개수가 55개라면, 재현율은 55/60 = 0.92 입니다. 
...
- 스팸을 분류할때 재현율을 중시한다면, 결과에서 스펨의 비율이 높아질수 있어 바람직하지 
...
- 스펨을 예로 들면, 스팸으로 판정한 메일 중에서 진짜 스펨의 비율에 해당합니다. 전체 메일 100개 중에 스펨으로 판정한 메일이 80개이고 진짜 스펨이 55개였다면, 정밀도는 55/80 = 0.68 입니다. 스펨 메일은 삭제되므로 이 비율이 낮으면 낮은 평가를 받게 됩니다. 
