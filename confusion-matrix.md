...
- 스펨을 예로 들면, __실제 스팸중에 진짜 스펨의 비율__에 해당합니다. 전체 메일 중에 실제 스펨이 60개이고, 스펨으로 판정한 메일 개수가 55개라면, 재현율은 55/60 = 0.92 입니다. 
...
- 스펨을 예로 들면, _스팸으로 판정한 메일 중에서 진짜 스펨의 비율_에 해당합니다. 전체 메일 중에 스펨으로 판정한 메일이 80개이고 진짜 스펨이 55개였다면, 정밀도는 55/80 = 0.68 입니다. 
